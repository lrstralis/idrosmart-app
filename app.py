...
with tab_home:
    ...
    with c_h2: 
        # Usiamo un piccolo form anche per il cambio data veloce per bloccare l'auto-reload istantaneo al click del calendario
...
with tab_dashboard:
    ...
    # BLOCCO 1: FORM DI INSERIMENTO COMPLETO NELLA SIDEBAR PER BLOCCARE I CARICAMENTI AD OGNI CLICK DEI MENU
    st.sidebar.header("➕ Nuovo Turno")
    with st.sidebar.form("form_nuovo_turno"):
        tipo_elemento_scelto = st.radio("Tipo Elemento:", ["Agricoltori", "Chiavoni"], horizontal=True)
        ...
        irrigante_scelto = st.selectbox("Seleziona Contadino / Chiavone", opzioni_sb)
        tipo_pesca_scelta = st.radio("Modalità Prelievo", ["Fosso", "Diretta"], index=1)
        motori_scelti_sb = st.number_input("Motori da far uscire (M):", min_value=0.0, max_value=12.0, value=1.0, step=0.01)
    ...
    if salva_bottone:
        if not irrigante_scelto or irrigante_scelto == "Nessun elemento registrato":
            ...
            if id_irrigante_db is not None:
                aggiorna_irrigante_completo(id_irrigante_db, irrigante_scelto, irrigante_scelto, tipo_pesca_scelta, motori_scelti_sb, 30, 15, 0)
            else:
                id_irrigante_db = inserisci_irrigante_completo(irrigante_scelto, irrigante_scelto, tipo_pesca_scelta, motori_scelti_sb, 30, 15, 0)
    ...
    # BLOCCO 2: DANGER ZONE CON FORM CONTRO I RALLENTAMENTI
    st.sidebar.markdown("---")
...
# =========================================================
# TAB 4: GESTIONE ANAGRAFICA (TUTTO RACCHIUSO NEI FORM)
# =========================================================
with tab_anagrafica:
    ...
    with sub_ins:
        # Tutto raggruppato in un unico Form per evitare ricariche mentre scrivi o selezioni i menu a tendina
    ...
    with sub_mod:
        if df_irriganti.empty: 
            ...
            # Selezione profilo (fuori dal form per caricare i dati della scheda corretta)
